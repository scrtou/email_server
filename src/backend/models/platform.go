...
type Platform struct {
	gorm.Model
	UserID     uint   `gorm:"not null;index:idx_user_platform_name,unique"` // 外键，关联到 User 模型
	Name       string `gorm:"type:varchar(255);not null;index:idx_user_platform_name,unique"` // 平台名称, 用户ID和平台名称组合唯一
